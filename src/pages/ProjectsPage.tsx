...
function ProjectCard({ project, index }: { project: Project; index: number }) {
  ...
  const getProjectType = () => {
    ...
    if (tags.some((t: string) => t.includes('ai') || t.includes('ml') || t.includes('llm'))) return 'AI'
    if (tags.some((t: string) => t.includes('react') || t.includes('vue') || t.includes('frontend'))) return 'Frontend'
    if (tags.some((t: string) => t.includes('node') || t.includes('python') || t.includes('backend'))) return 'Backend'
    if (tags.some((t: string) => t.includes('cli') || t.includes('tool'))) return 'Tool'
    return 'FullStack'
  }
            ...
            >
              {project.demoUrl && (
                <a
                  ...
                  className="flex items-center gap-2 px-6 py-3 bg-white text-black rounded-full font-medium hover:bg-gray-100 transition-colors"
                >
              ...
              {project.repoUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-2 px-6 py-3 bg-black/80 text-white rounded-full font-medium hover:bg-black transition-colors"
                >
