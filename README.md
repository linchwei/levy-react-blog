...
项目包含一个智能 Git 提交脚本 [`git-commit.sh`](./git-commit.sh)，能自动分析修改的文件并生成符合规范的提交信息。

```bash
# 基本用法
./git-commit.sh                    # 自动提交所有修改
./git-commit.sh "修复登录bug"      # 使用自定义提交信息
./git-commit.sh --push             # 提交并推送到远程
./git-commit.sh -y                 # 自动确认，不提示
./git-commit.sh -h                 # 显示帮助
```

**智能提交信息生成规则：**
- 自动识别文件类型（组件、页面、Hook、样式等）
- 根据修改内容生成提交类型（feat/fix/docs/style/chore等）
- 统计各类修改数量并附加到提交信息
- 示例：`feat: 更新Gallery组件 [功能(3) 样式(2) ]`
